...
Shader "Toony Colors Pro 2/Examples/Water/Reflection_zyx"
{
	Properties
	{
	...
	SubShader
	{
		...
		#pragma surface surf ToonyColorsWater keepalpha vertex:vert nolightmap finalcolor:fcolor noambient
		#pragma target 3.0
		...
		inline float4 LightingToonyColorsWater(inout SurfaceOutputWater s, float3 viewDir, UnityGI gi)
		{
			...
			float ndl = max(0, dot(s.Normal, lightDir));
			#define NDL ndl
			#define	RAMP_THRESHOLD	_RampThreshold
			#define	RAMP_SMOOTH		_RampSmooth

			float3 ramp = smoothstep(RAMP_THRESHOLD - RAMP_SMOOTH*0.5, RAMP_THRESHOLD + RAMP_SMOOTH*0.5, NDL);
			#if !defined(UNITY_PASS_FORWARDBASE)
				_SColor = float4(0,0,0,1);
			#endif
		...
		void fcolor(Input IN, SurfaceOutputWater o, inout fixed4 color) {
		#ifdef ENABLE_FOG
			float3 fogColor = _FogColor.rgb;
		#ifndef UNITY_PASS_FORWARDBASE
			fogColor = 0;
		#endif
			float fogVar = saturate(1.0 - (_FogEnd - IN.viewZ) / (_FogEnd - _FogStart));
			color.rgb = lerp(color.rgb, fogColor, fogVar);
		#endif
		}
	...
	Fallback "Diffuse"
	CustomEditor "TCP2_MaterialInspector_Water"
}
