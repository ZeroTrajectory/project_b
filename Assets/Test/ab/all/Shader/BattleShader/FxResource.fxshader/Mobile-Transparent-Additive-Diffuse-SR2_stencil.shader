...
Shader "Mobile/Transparent/Additive/Diffuse-SR2_stencil" {
Properties {
...
SubShader {
	...
	Pass {
		...
		fixed4 frag (v2fself i) : COLOR
		{
			fixed4 color = tex2D(_MainTex, i.uv.xy) * tex2D(_DetailTex, i.uv.zw) * i.color;
			v2f v2fParent;
			v2fParent.worldPosition = i.worldPosition;
			color = calculateFadeRectMask(v2fParent, color);
			return color;
		}
		ENDCG 
	}	
