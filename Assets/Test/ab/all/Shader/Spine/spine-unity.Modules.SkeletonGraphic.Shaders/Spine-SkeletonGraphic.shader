...
Shader "Spine/SkeletonGraphic (Premultiply Alpha)"
{
	Properties
	{
		...
		[Space(50)]
		[Toggle(_TEX_MODE_ADD)] _TEX_MODE_ADD("Is texture mode add", Int) = 0
		_CanvasAlpha("CanvasAlpha(add mode)", Float) = 1
	}
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			#pragma multi_compile __ _TEX_MODE_ADD
	...
	fixed4 Blur(sampler2D tex, half2 uv, float size)
	{
		...
		fixed4 frag (VertexOutput IN) : SV_Target
		{
			...
			half4 totalColor;
				...
				{
					// 注 ID1380554梅丝忒立绘有线条纹 的修复方案
					// 早期版本spine资源的边缘不存在rgb==1且a接近于0的情况
					// 近期改版spine资源的边缘均为rgb==1且a接近于0,导致边缘增加条纹时本应接近无色但却显示为高亮
					// 因此在处理横向条纹时做纹理的alpha加权处理
					totalColor.rgb = totalColor.rgb + half3(0.1686, 0.2196, 0.3215) * totalColor.a;//横向条纹
				}
			...
			#ifdef _TEX_MODE_ADD
				color.rgb *= _CanvasAlpha * _CanvasAlpha;
				color.a *= _CanvasAlpha;
			#else
			...
			color *= UnityGet2DClipping(IN.worldPosition.xy, _ClipRect);
				...
				#ifndef _TEX_MODE_ADD 
					clip (color.a - 0.001);
				#endif
			#endif

			//color = lerp(IN.color2, color, IN.color.r);
